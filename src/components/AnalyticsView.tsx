...
const COLORS = ['hsl(160, 84%, 30%)', 'hsl(38, 92%, 50%)', 'hsl(200, 70%, 50%)', 'hsl(280, 60%, 50%)', 'hsl(0, 72%, 51%)', 'hsl(120, 50%, 40%)', 'hsl(30, 80%, 55%)', 'hsl(180, 60%, 40%)'];
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={monthlyData}>
                ...
                <Tooltip contentStyle={{ background: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', borderRadius: '8px' }} formatter={(v: number) => [`€${v.toFixed(2)}`, 'Total']} />
                <Line type="monotone" dataKey="total" stroke="hsl(160, 84%, 30%)" strokeWidth={2} dot={{ fill: 'hsl(160, 84%, 30%)' }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </CardContent>
              ...
              {merchantData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={merchantData} layout="vertical">
                    ...
                    <Bar dataKey="value" fill="hsl(38, 92%, 50%)" radius={[0, 6, 6, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              ) : <div className="h-full flex items-center justify-center text-muted-foreground text-sm">No data</div>}
            </div>
